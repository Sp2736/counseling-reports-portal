...
import { User, ShieldCheck, AlertCircle, ArrowLeft, Save } from "lucide-react";

export default function StudentProfilePage() {
  const router = useRouter();
  
  const [fullName, setFullName] = useState("");
  const [studentId, setStudentId] = useState(""); // We keep this read-only
  const [department, setDepartment] = useState("");
  const [assignedCounselor, setAssignedCounselor] = useState("");
  
  const [counselors, setCounselors] = useState<{_id: string, fullName: string}[]>([]);
  ...
  useEffect(() => {
    fetch("/api/student/profile")
      ...
      .then((data) => {
        ...
        setAssignedCounselor(data.profile.assignedCounselor);
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/student/profile", {
        ...
        body: JSON.stringify({ fullName, department, assignedCounselor }),
      });
