...
import { ArrowLeft, Bot, Save, CheckCircle, University, ShieldCheck, FileText } from "lucide-react";
...
export default function CounselorRecordReview({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (isLoading) return <div className="p-8 text-center text-slate-500">Generating Live Document...</div>;
          ...
          <div className="lg:col-span-8 bg-white border border-gray-300 shadow-xl p-8 md:p-12 text-black max-w-[800px] mx-auto w-full">
            ...
            <div className="flex justify-between items-start border-b-2 border-slate-900 pb-6 mb-8">
              <div>
                <div className="flex items-center space-x-3 mb-2">
                  ...
                  <h1 className="text-2xl font-black tracking-tight uppercase text-slate-900">Student Counseling</h1>
                </div>
                {/* NEW: Updated Sub-Title */}
                <h2 className="text-sm font-bold text-slate-500 uppercase tracking-widest">SWOT Analysis Report</h2>
              </div>
              <div className="text-right text-xs text-slate-600 space-y-1.5 font-mono">
                ...
                <p><span className="font-bold text-slate-400">STUDENT ID:</span> {record.student?.studentId}</p>
                <p><span className="font-bold text-slate-400">DATE OF APPROVAL:</span> {new Date(record.createdAt).toLocaleDateString()}</p>
              </div>
            </div>
            ...
            {isReviewed && (
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg mb-8 flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-green-800 mb-1">Document Status</p>
                  <p className="text-sm font-bold text-green-900">Reviewed & Finalized</p>
                </div>
            ...
            <div className="space-y-10">
              ...
              <section>
                <h3 className="text-sm font-black uppercase tracking-widest border-b border-slate-200 pb-2 mb-4 text-slate-900 flex items-center">
                  <span className="bg-slate-900 text-white w-5 h-5 inline-flex items-center justify-center rounded-full mr-2 text-[10px]">1</span> 
                  Self-Assessment Data
                </h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 p-4 rounded-md border border-slate-100">
                    <h4 className="font-bold text-slate-800 mb-2 text-xs uppercase">Strengths</h4>
                    <ul className="text-xs text-slate-600 space-y-1 list-disc list-inside">
                      {swot.strengths?.map((item: string, i: number) => <li key={i}>{item}</li>)}
                    </ul>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-md border border-slate-100">
                    <h4 className="font-bold text-slate-800 mb-2 text-xs uppercase">Weaknesses</h4>
                    <ul className="text-xs text-slate-600 space-y-1 list-disc list-inside">
                      {swot.weaknesses?.map((item: string, i: number) => <li key={i}>{item}</li>)}
                    </ul>
                  </div>
                </div>
              </section>
              ...
              {ai && (
                <section>
                  <h3 className="text-sm font-black uppercase tracking-widest border-b border-slate-200 pb-2 mb-4 text-slate-900 flex items-center">
                    <span className="bg-slate-900 text-white w-5 h-5 inline-flex items-center justify-center rounded-full mr-2 text-[10px]">2</span> 
                  ...
                  <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-sm bg-slate-50 p-5 rounded-md border border-slate-100">
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Learning Style</p>
                      <p className="text-slate-800 font-medium">{ai.psychological_profile?.learning_style}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Growth Category</p>
                      <p className="text-slate-800 font-medium">{ai.psychological_profile?.psychological_growth_category}</p>
                    </div>
                  </div>
                </section>
              )}
              ...
              <section className="break-inside-avoid">
                <h3 className="text-sm font-black uppercase tracking-widest border-b border-indigo-200 pb-2 mb-4 text-indigo-900 flex items-center">
                  <span className="bg-indigo-600 text-white w-5 h-5 inline-flex items-center justify-center rounded-full mr-2 text-[10px]">3</span> 
                  Official Action Plan (Counselor Input)
                </h3>
                
                <div className="bg-indigo-50/50 border border-indigo-100 p-6 rounded-lg space-y-6">
                  <div>
                    <label className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest block mb-2">Final Assessed Risk Level</label>
                    <select 
                    ...
                    >
                      ...
                      <option value="Critical">Critical Intervention Required</option>
                    </select>
                  </div>
                  
                  <div>
                    <label className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest block mb-2">Prescribed Interventions & Notes</label>
                    <textarea 
                      rows={8}
                      required
            ...
            <div className="mt-16 pt-6 flex justify-between items-end opacity-60">
              ...
              <div className="text-center w-1/3">
                <p className="text-[10px] text-slate-800 font-bold mt-1 uppercase truncate">{record.student?.fullName || "Student"}'s Signature</p>
              </div>

              {/* Counselor Signature Block */}
              <div className="text-center w-1/3">
                <p className="text-[10px] text-slate-800 font-bold mt-1 uppercase truncate">{record.assignedCounselor?.fullName || "Counselor"}'s Signature</p>
              </div>
