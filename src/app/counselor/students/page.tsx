    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center space-x-4 mb-6">
          <button onClick={() => router.push("/counselor/students")} className="p-2 bg-white rounded-full shadow-sm hover:bg-gray-100 transition-colors">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100">
                {records.length === 0 ? (
                  <tr><td colSpan={4} className="p-8 text-center text-gray-500">This student has no reviewed reports yet.</td></tr>
                ) : (
                  records.map((record) => (
                    <tr key={record._id} className="hover:bg-gray-50 transition-colors">
                      <td className="p-4 text-gray-900 font-medium">{new Date(record.createdAt).toLocaleDateString()}</td>
                      <td className="p-4">
